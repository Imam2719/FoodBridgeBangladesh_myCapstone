...
public class MerchantController {
    ...
    public ResponseEntity<?> addMerchant(@ModelAttribute MerchantDTO merchantDTO) {
        try {
            ...
            response.put("data", addedMerchant);
            ...
            if (errorMessage != null) {
                ...
                } else if (errorMessage.contains("Business license number already registered")) {
                    response.put("message", "This business license number is already registered");
                    return ResponseEntity.status(HttpStatus.CONFLICT).body(response);
                } else if (errorMessage.contains("Invalid file format")) {
                    response.put("message", "Invalid file format. Please upload valid image/document files");
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
                }
            }
            
            // Default error message
            response.put("message", "Failed to add merchant: " + errorMessage);
