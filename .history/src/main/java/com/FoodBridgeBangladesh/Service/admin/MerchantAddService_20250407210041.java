...
public class MerchantAddService {
    ...
    public MerchantEntity addMerchant(MerchantDTO merchantDTO) throws Exception {
        try {
            logger.info("Starting merchant registration process with data: " + merchantDTO);
            ...
            try {
                ...
                logger.severe("Database error while saving merchant: " + e.getMessage());
                throw new RuntimeException("Failed to save merchant to database", e);
            }

        } catch (Exception e) {
            logger.severe("Error in addMerchant: " + e.getMessage());
            e.printStackTrace();
            throw new RuntimeException("Failed to process merchant registration: " + e.getMessage(), e);
        }
    }
