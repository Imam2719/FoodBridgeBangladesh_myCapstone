...
const foodIcons = [
  ...
  { icon: Sandwich, color: "#d97706" },
  { icon: Cookie, color: "#a8815c" },
  { icon: Salad, color: "#84cc16" },
  { icon: UtensilsCrossed, color: "#6b7280" },
  { icon: Carrot, color: "#f97316" },
  { icon: Beef, color: "#b91c1c" },
  { icon: Fish, color: "#3b82f6" },
  { icon: Egg, color: "#fbbf24" },
  { icon: Cheese, color: "#fcd34d" },
  { icon: Cherry, color: "#e11d48" },
  { icon: Banana, color: "#fbbf24" }
];
...
const LoginPage = () => {
  ...
  // Generate random position for icons
  const generateRandomPosition = () => {
    return {
      left: `${Math.random() * 100}%`,
      top: `${Math.random() * 100}%`,
      rotate: `${Math.random() * 360}deg`,
      size: Math.floor(Math.random() * 20) + 15, // Size between 15-35px
      duration: Math.floor(Math.random() * 20) + 20, // Animation duration 20-40s
      delay: Math.floor(Math.random() * 5),
      opacity: Math.random() * 0.15 + 0.05 // Opacity between 0.05-0.2 (more subtle)
    };
  };
  ...
  useEffect(() => {
    const generateIcons = () => {
      const icons = [];
      // Generate 40 random food icons (more icons but less visible)
      for (let i = 0; i < 40; i++) {
        const iconData = foodIcons[Math.floor(Math.random() * foodIcons.length)];
        icons.push({
          id: `icon-${i}`,
          ...iconData,
    ...
    // Regenerate some icons every 15 seconds to keep the background dynamic
    const interval = setInterval(() => {
      setBackgroundIcons(prevIcons => {
        // Replace 8 random icons with new ones
        const newIcons = [...prevIcons];
        for (let i = 0; i < 8; i++) {
          const randomIndex = Math.floor(Math.random() * newIcons.length);
          const iconData = foodIcons[Math.floor(Math.random() * foodIcons.length)];
          newIcons[randomIndex] = {
            id: `icon-${randomIndex}-${Date.now()}`,
            ...iconData,
            ...generateRandomPosition()
          };
        }
  ...
  return (
    <div className={`auth-page ${darkMode ? 'dark-mode' : ''}`}>
      {/* FoodBridge Logo */}
      <div className="foodbridge-logo">
        <span>FoodBridge</span>
        <span className="location">Bangladesh</span>
      </div>

      {/* Theme Toggle and Language Controls - Placeholders */}
      <div className="header-controls">
        <button className="lang-btn">
          <span className="globe-icon">🌐</span>
        </button>
        <button className={`theme-toggle ${darkMode ? 'dark' : 'light'}`}>
          {darkMode ? '☀️' : '🌙'}
        </button>
      </div>

      {/* Dynamic food icons background */}
      <div className="food-icons-background">
        ...
        <div className={`auth-overlay ${darkMode ? 'dark-overlay' : 'light-overlay'}`}></div>
      </div>
