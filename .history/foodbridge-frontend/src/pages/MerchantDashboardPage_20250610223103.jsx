...
const handleSendNewMessage = async (e) => {
  ...
  try {
    ...
    formData.append('recipientType', newMessageData.recipientType || 'admin');
    formData.append('senderRole', 'merchant'); // Set role as merchant
    ...
    if (response.ok) {
      ...
      setNewMessageData({ toEmail: '', subject: '', message: '', recipientType: 'admin' });
      setRecipient('admin');
...
const NewMessageModal = () => {
  const [recipient, setRecipient] = useState('admin'); // 'admin' or 'donor'
  ...
  const handleRecipientChange = (selectedRecipient) => {
    ...
    // Auto-fill email based on recipient
    ...
    } else if (selectedRecipient === 'donor') {
      setNewMessageData({
        ...newMessageData,
        toEmail: '',
        recipientType: 'donor'
          ...
          <div className="space-y-4">
              ...
              <div className="flex space-x-4">
                ...
                <label className="flex items-center">
                  <input
                    ...
                    name="recipient"
                    value="donor"
                    checked={recipient === 'donor'}
                    onChange={() => handleRecipientChange('donor')}
                    className="mr-2"
                  />
                  <span>Donor</span>
                </label>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  // Only allow editing if recipient is donor
                  if (recipient === 'donor') {
                    setNewMessageData({ ...newMessageData, toEmail: e.target.value });
                  }
                }}
                placeholder={recipient === 'admin' ? 'sfms0674@gmail.com' : 'Enter donor email'}
                required
                readOnly={recipient === 'admin'}
                className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  recipient === 'admin' ? 'bg-gray-100 cursor-not-allowed' : ''
                }`}
              />
              {recipient === 'admin' && (
                <small className="text-gray-500 mt-1 block">Admin email is auto-selected and cannot be changed</small>
              )}
            </div>
      ...
      <div className="popup-header">
        ...
        <div className="header-actions">
          <button
            ...
            onClick={() => {
              setMessagesOpen(false); // Close messages popup first
              setTimeout(() => setShowNewMessageModal(true), 100); // Then open new message modal
            }}
          >
            <Plus size={18} />
        ...
        {messagesLoading ? (
          ...
          <div className="empty-state">
            ...
            <button
              ...
              onClick={() => {
                setMessagesOpen(false);
                setTimeout(() => setShowNewMessageModal(true), 100);
              }}
            >
              <Plus size={18} />
