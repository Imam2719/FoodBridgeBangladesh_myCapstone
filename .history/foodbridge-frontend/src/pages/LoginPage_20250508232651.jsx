...
const FoodIcon = ({ icon: Icon, size, color, style, className }) => (
  <div className={`food-icon ${className || ''}`} style={style}>
...
const LoginPage = () => {
  ...
  // Generate random food icons for the background
  const generateFoodIcons = () => {
    const icons = [];
    const foodIconComponents = [Apple, Coffee, Pizza, Grape, Beef, Carrot, Sandwich];
    const iconColors = darkMode 
      ? ['#f97316', '#84cc16', '#14b8a6', '#f43f5e', '#8b5cf6', '#ec4899', '#f59e0b'] 
      : ['#ea580c', '#65a30d', '#0d9488', '#be123c', '#7c3aed', '#db2777', '#d97706'];
    
    // Create 30 random food icons
    for (let i = 0; i < 30; i++) {
      const randomIcon = foodIconComponents[Math.floor(Math.random() * foodIconComponents.length)];
      ...
      const randomTop = Math.floor(Math.random() * 100); // 0-100%
      const randomDuration = Math.floor(Math.random() * 20) + 30; // 30-50s
      const randomDelay = Math.floor(Math.random() * 15); // 0-15s
  ...
  useEffect(() => {
    setFoodIcons(generateFoodIcons());
  }, [darkMode]);
  ...
  return (
    <div className={`auth-page ${darkMode ? 'dark-mode' : ''}`}>
      {/* Dynamic food icon background */}
      <div className="food-icon-background">
        {foodIcons}
