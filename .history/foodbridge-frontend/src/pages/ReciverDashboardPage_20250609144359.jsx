...
const ReceiverDashboard = () => {
  ...
  const [savedFoods, setSavedFoods] = useState([2, 4, 6]); // IDs of saved foods
...
const [showActionButtons, setShowActionButtons] = useState(true);
  ...
  // Call this in the useEffect hook
  useEffect(() => {
    if (currentUser.id) {
      fetchUserRequests();
    }
  ...
  const dietaryOptions = [
    "Vegetarian", "Vegan", "Halal", "Gluten-Free", "Nut-Free",
    "Dairy-Free", "Organic", "Low-Carb", "High-Protein", "Child-Friendly"
  ];
  ...
  const fetchAvailableFoodDonations = async () => {
    try {
      ...
      // Fix: Remove forced null for category parameter
      const response = await axios.get(`${API_BASE_URL}/api/receiver/food/available`, {
        params: {
          category: selectedCategory !== 'All' ? selectedCategory : null,
          maxDistance: maxDistance,
          dietaryFilters: selectedDietaryFilters.length > 0 ? selectedDietaryFilters : null,
  ...
  }, [selectedCategory, selectedDietaryFilters, maxDistance, pagination.currentPage]);
  ...
  const toggleSaveFood = (id) => {
    ...
    const notification = document.createElement('div');
    notification.className = 'notification-dropdown';
    const isSaving = !savedFoods.includes(id);
    notification.innerHTML = `<div class="flex items-center p-3"><${isSaving ? 'Bookmark' : 'X'} class="h-5 w-5 mr-2 text-${isSaving ? 'blue' : 'red'}-500" /><span>${isSaving ? 'Food saved to your collection!' : 'Food removed from saved items'}</span></div>`;
    document.body.appendChild(notification);
  ...
  // Modify existing filter-related methods
          ...
          <div className="card-content dark:border-gray-700">
            ...
            <div className="card-actions border-t border-gray-100 dark:border-gray-700">
              ...
              <button
                ...
                className="btn-info dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
              >
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="p-4 md:w-2/3 flex flex-col justify-between dark:border-gray-700">
              ...
              <div className="flex space-x-2">
                ...
                <button
                  onClick={() => handleViewDetails(food.id)}
                  className="px-3 py-2 rounded-lg text-sm bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 flex items-center justify-center transition"
                >
                  <Info className="h-4 w-4" />
                </button>
              </div>
      ...
      >
        ...
        {availableFoods.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center p-10 bg-white dark:bg-gray-800 rounded-xl shadow-sm mt-6">
            ...
            <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-2">No Active Food Donations Available</h3>
            <p className="text-gray-500 dark:text-gray-400 max-w-md text-center">
              There are currently no active food donations available. Please check back later or adjust your filters.
            </p>
