...
const MerchantDashboard = () => {
  ...
  recipientType: 'admin', // 'admin' or 'donor'
  toEmail: 'sfms0674@gmail.com',
  subject: '',
  message: '',
  attachment: null
});
  ...
  // Send new message
const handleSendNewMessage = useCallback(async (e) => {
  e.preventDefault();
  e.stopPropagation();
  ...
  try {
    ...
    formData.append('recipientType', newMessageData.recipientType);

    // Add attachment if present
    if (newMessageData.attachment) {
      formData.append('attachment', newMessageData.attachment);
    }
...
const NewMessageModal = () => {
  ...
  // FIXED: Use useCallback to prevent re-renders on every keystroke
  const handleSubjectChange = useCallback((e) => {
    setNewMessageData(prev => ({ ...prev, subject: e.target.value }));
  }, []);

  const handleMessageChange = useCallback((e) => {
    setNewMessageData(prev => ({ ...prev, message: e.target.value }));
  }, []);

  const handleEmailChange = useCallback((e) => {
    setNewMessageData(prev => ({ ...prev, toEmail: e.target.value }));
  }, []);
  ...
  const handleRecipientTypeChange = useCallback((type) => {
    setNewMessageData(prev => ({
      ...prev,
      recipientType: type,
      toEmail: type === 'admin' ? 'sfms0674@gmail.com' : '',
      subject: '', // Reset subject when changing type
      message: '',
      attachment: null
    }));
  }, []);
  ...
  const handleFileChange = useCallback((e) => {
    ...
    if (file) {
      ...
      setNewMessageData(prev => ({ ...prev, attachment: file }));
    }
  }, []);

  const removeAttachment = useCallback(() => {
    setNewMessageData(prev => ({ ...prev, attachment: null }));
        ...
        <div className="message-modal-content">
          ...
          <div className="message-modal-header">
            <div className="modal-header-content">
              ...
              <p className="modal-subtitle">
                {newMessageData.recipientType === 'admin' 
                  ? 'Sending message to Admin Support' 
                  : 'Sending message to Food Donors'
                }
              </p>
            </div>
            <button 
            ...
            <form onSubmit={handleSendNewMessage} className="message-form">
              ...
              <div className="form-section">
                ...
                <div className="recipient-type-container">
                  <label className={`recipient-type-card ${newMessageData.recipientType === 'admin' ? 'selected' : ''}`}>
                    <input
                      type="radio"
                      name="recipientType"
                      value="admin"
                      checked={newMessageData.recipientType === 'admin'}
                    ...
                    <div className="recipient-card-content">
                      ...
                      <div className="recipient-info">
                        ...
                        <span className="recipient-desc">Technical support, account issues, business queries</span>
                      </div>
                    </div>
                  </label>
                  
                  <label className={`recipient-type-card ${newMessageData.recipientType === 'donor' ? 'selected' : ''}`}>
                    <input
                      type="radio"
                      name="recipientType"
                      value="donor"
                      checked={newMessageData.recipientType === 'donor'}
                      onChange={(e) => handleRecipientTypeChange(e.target.value)}
                      className="recipient-radio"
                    />
                    <div className="recipient-card-content">
                      ...
                      <div className="recipient-info">
                        ...
                        <span className="recipient-desc">Partnership proposals, donation inquiries</span>
                      </div>
                    </div>
                  </label>
                </div>
              ...
              <div className="form-section">
                <label htmlFor="toEmail" className="field-label">
                  To Email* 
                  {newMessageData.recipientType === 'admin' && (
                    <span className="field-note">(Auto-filled, cannot be changed)</span>
                  )}
                </label>
                <div className="email-input-container">
                  <input
                    ...
                    placeholder={newMessageData.recipientType === 'admin' ? "Admin Email (Auto-filled)" : "Enter donor email address"}
                    required
                    readOnly={newMessageData.recipientType === 'admin'}
                    className={`form-input ${newMessageData.recipientType === 'admin' ? 'readonly-input' : ''}`}
                    autoComplete="email"
                  />
                  {newMessageData.recipientType === 'admin' && (
                    <div className="input-lock-icon">
                      <Settings size={16} />
                    </div>
                  )}
                </div>
              ...
              <div className="form-section">
                ...
                <input
                  ...
                  placeholder={newMessageData.recipientType === 'admin' 
                    ? "e.g., Account Issue, Technical Support, Business Query" 
                    : "e.g., Food Donation Inquiry, Partnership Proposal"
                  }
                  required
                ...
                <div className="field-helper">
                  <span className="char-count">{newMessageData.subject.length}/100</span>
                </div>
              </div>
              ...
              <div className="form-section">
                ...
                <div className="textarea-container">
                  ...
                  <div className="textarea-helper">
                    <span className="char-count">{newMessageData.message.length}/2000</span>
                  </div>
                </div>
              </div>
                  ...
                  <div className="file-selected-display">
                    <div className="file-info">
                      ...
                      <div className="file-details">
                        <span className="file-name">{newMessageData.attachment.name}</span>
                        <span className="file-size">{formatFileSize(newMessageData.attachment.size)}</span>
                      </div>
                    </div>
                    <button
                      type="button"
          ...
          <div className="message-modal-footer">
            ...
            <div className="footer-note">
              <span>Messages are encrypted and securely delivered</span>
            </div>
          </div>
        </div>
      </div>
  ...
  setProfileOpen(false);
  setFeesOpen(false);
  setSalesHistoryOpen(false); // Close sales history
  setDonationsOpen(false);    // Close donations
      ...
      <div className="popup-header">
        ...
        <div className="header-actions">
          <button
            ...
            onClick={() => {
              setMessagesOpen(false); // Close messages popup
              setShowNewMessageModal(true); // Open new message modal
            }}
          >
            <Plus size={18} />
      ...
      <div className="popup-body messages-container">
        {messagesLoading ? (
          ...
          <div className="messages-layout">
            {/* Display messages here - implement as needed */}
